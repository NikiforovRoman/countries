...
const Detail = ({ name, flags, capital, population, region, subregion, tld, currencies = {}, languages = {}, borders = []}) => {
    ...
    useEffect(() => {
        if (borders.length) {
            axios.get(filterByCode(borders)).then(({data}) => setNeighbours((data.map(c => c.name.common))));
        }
    }, [borders])
    return (
                ...
                <div className={s.detail__data}>
                    ...
                    <div className={s.detail__lists}>
                        ...
                        <ul className={s.detail__list}>
                            ...
                            <li className={s.detail__item}>
                                <b>Currencies:</b> {currencyArray.map(c => <div key={c.name}>{c.name} ({c.symbol})</div>)}
                            </li>
                            <li className={s.detail__item}>
                    ...
                    <div className={s.detail__borders}>
                        ...
                        (
                            <div className={s.detail__buttons}>
                                {neighbours.map(b => (
                                    <button className={s.detail__button} key={b} onClick={() => push(`/country/${b}`)}>{b}</button>
                                ))}
                            </div>
                        )}
                    </div>
